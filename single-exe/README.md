...
It is still the last import in the file, which is all `ASSETS_BUNFS=1` needs to
find the main program. In that mode `gotty.js` is compiled as the entry point
itself, Bun rewrites the guard to `import.meta.main` — true — and it starts
itself; the exported `bootstrap` then simply goes unused.
