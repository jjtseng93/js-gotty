...
the executable carries its own `static/`, templates, documentation and so on.

Assets are declared once, in `package.json`:

```json
{
  "assets": ["static", "README.md", "README.en.md"]
}
```
...
Text compresses well, so the tar usually wins on size by a wide margin — for
jsgotty most of the payload is `static/`, whose JavaScript and CSS shrink to a
fifth. What it costs is paid on every run, whether or not the program touches a
single asset. Reach for `ASSETS_BUNFS=1` when that startup work, or holding the
whole payload in memory, matters more than the extra megabytes on disk.
...
const index = await readAssetText("static/index.html");

//  Listing does not, so pick the source.
const staticDirs = hasInternalAssets()
  ? listInternalAssetDirs("static")
  : readdirSync(join(REPO_ROOT, "static"));
```
...
called instead, and that call has to survive. jsgotty is exactly that case; see
[below](#the-entry-import-calls-an-exported-starter).
...
`entry.mjs` is immune because it writes its own, and one is enough. Every other
`await` in the graph — all of `assetsLoader.mjs`'s, for one — sits inside a
function and cannot help. It is also why bunfs builds compile a copy of that
file rather than handing the main program to `bun build` directly: a main
program that reaches top-level await only through its imports writes none of
its own.
...
(2026-08-22). Until that changes, treat entry.mjs's `await`s as API.
...
```shell
bun ./gotty.js --build-exe --sourcemap
bun ./gotty.js --build-for bun-linux-x64 --sourcemap
```
...
bun --define MY_APP=../app.md ./gotty.js --build-exe

# Right
bun ./gotty.js --build-exe --define MY_APP=../app.md
```
...
The rest of this file is generic. These names belong to jsgotty itself and are
not reserved by the bootstrap — an adopting project picks its own.

Building is optional; running `bun gotty.js` from a checkout, as the root
`README.md` describes, remains the usual way to run jsgotty.

```shell
bun ./gotty.js --build-exe
bun ./gotty.js --build-for bun-linux-x64
```

Both produce `jsgt`: `gotty.js` calls `buildEarlyExit(process.argv, "jsgt")`
from `bootstrap()`, before any CLI parsing, so the build switches exit before
a server is ever started. Neither works from an already compiled binary.

The three packed entries — `static`, `README.md`, `README.en.md` — are what the
HTTP server hands out: `index.html`, `manifest.json`, `help.md`, the frontend
JavaScript and CSS, and the two READMEs behind the help pages.

### The entry import calls an exported starter

`gotty.js` is CommonJS and starts itself from `require.main === module`. With
`entry.mjs` as the bundle's entry point that guard is inlined to `false`, so
the import cannot rely on it and calls the export instead:
...
Both back ends go through it: `ASSETS_BUNFS=1` compiles a copy of `entry.mjs`,
so `gotty.js` is never the entry point and the guard stays `false` in either
mode. Handing `gotty.js` to `bun build` directly instead would compile it as
the entry, Bun would rewrite the guard to `import.meta.main` — true — and it
would start itself; that path is not taken, and `bootstrap` is what starts the
program in both builds.

### `packAssets.sh`

The original implementation, kept as a reference: a hand-written tar of the
three asset entries, from before `assetsPacker.js` and its `package.json`
declaration existed. `buildExecutable()` in `compiled.js` is the matching
build path and still runs it, but `buildExe` now points at `buildExeAssets`,
so nothing in the current flow calls either. It is the shortest way to see
what the packer produces, and it still packs an `assets.tar` by hand:

```shell
bun ./single-exe/packAssets.sh
bun build --compile --bytecode --minify ./single-exe/entry.mjs --outfile=jsgt
```
